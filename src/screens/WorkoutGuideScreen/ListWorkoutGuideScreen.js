...
const ExpandableComponent = ({ item, onClickFunc }) => {
  ...
  const onCheckPressed = (item) => {
    navigation.navigate('Workout Guide', {
      workoutGuide: item,
    });
  }
...
const ListWorkoutGuideScreen = ({route}) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`${apiUrl}listworkout&id=${userData.id}`);
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <View style={styles.root}>
        <Text style={styles.title}>List Workout Guide</Text>
      </View>
...
export default ListWorkoutGuideScreen;